...
module.exports = { isTable, getExtent, getGeomType, getHasZ, detectType, esriTypeMap }
...
function getHasZ (geojson = {}) {
  // TODO this should find the first non-null geometry
  if (geojson.metadata && geojson.metadata.hasZ) return geojson.metadata.hasZ
  if (!geojson.features || !geojson.features[0]) return undefined
  const feature = geojson.features[0]
  if (feature && feature.geometry && feature.geometry.z) return true
  return false
}
